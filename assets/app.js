...
(function(){
  ...
  async function renderStandard(mainEl){
    // Prefer explicit data-src on the element (category pages). No default fallback.
    const dataSrc = mainEl?.dataset?.src || null;

    // If no data source and no in-memory data and no global JSON allowed, do nothing.
